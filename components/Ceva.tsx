...
export interface MyTextProps extends TextProps, Showable {
    nonewline?: boolean
    parseText?: boolean
}
...
export function MyText(props: MyTextProps) {
    ...
    if (props.parseText)
        return <Text  {...props}
                      style={[{color: defaultColor}, props.style]}>{parseItalicAndBold(props.children as string)}</Text>
    return <Text  {...props} style={[{color: defaultColor}, props.style]}>{props.children}</Text>
}

export interface MyTextInputProps extends TextInputProps, Showable {
    children?: string
}
